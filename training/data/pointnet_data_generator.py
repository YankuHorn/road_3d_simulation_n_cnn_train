...
import cv2
import os
from training.data.data_utils import img2points
import random
...
class PointNetDataGenerator(keras.utils.Sequence):
    'Generates data for Keras'
    def __init__(self, list_IDs, data_dir, labels_dir, batch_size=10, max_num_points=4096,
    ...
    def _load_labels(self, dir_name, ID, img_type='seg_resized', flip=False):
        ...
        if os.path.isfile(os.path.join(dir_name, label_ID)):
            ...
            # res_dict['horizon'] = json_data[horizon_key] * one_over_img_height
            res_dict['host_yaw_at_100m'] = 0  # json_data[host_yaw_at_100m_key] * one_over_img_width
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        X = np.zeros((len(list_IDs_temp), max_num_points, 3), dtype=np.int64)
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            points = self._load_data(self.data_dir, seg_img_ID, flip, max_num_points)
            num_points = min(len(points), max_num_points)
            X[i, :num_points] = points[:num_points]
            # Store class
