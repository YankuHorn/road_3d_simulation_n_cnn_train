...
import cv2
import os
from training.data.data_utils import img2points, img2_new_hybrid
import random
...
class My_new_hybrid_data_generator(keras.utils.Sequence):
    'Generates data for Keras'
    def __init__(self, list_IDs, data_dir, labels_dir, batch_size=10, max_num_columns=70,
    ...
    @staticmethod
    def md2vcl_flat(md):
        vcl_flat = np.asarray([md['vehicles_center_xs'], md['vehicles_center_bottom_ys'], md['vehicles_widths'], md['vehicles_heights']])
        vcl_flat = vcl_flat.flatten()
        return vcl_flat
    ...
    def _load_data(self, dir_name, ID, flip):
        ...
        md_ID = os.path.join(dir_name.replace('front_view_image', 'meta_data'), ID.replace('.png', '.json'))
        md_ID = md_ID.replace('seg_front_view_image', "meta_data")
        md = self.read_json(md_ID)
        hybrid_img = img2_new_hybrid(img, self.max_num_columns)
        vcl_flat_ = self.md2vcl_flat(md)
        vcl_flat_structured = np.zeros((30 * 4,))
        vcl_flat_structured[:len(vcl_flat_)] = vcl_flat_

        return hybrid_img, vcl_flat_structured
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        hybrid_images = np.zeros((len(list_IDs_temp), 288, self.max_num_columns, 3), dtype=np.int64)
        hybrid_vcls = np.zeros((len(list_IDs_temp), 30 * 4), dtype=np.float64)
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            hybrid_img, vcls_flat = self._load_data(self.data_dir, seg_img_ID, flip)
            # num_points = min(len(points), max_num_points)
            hybrid_images[i, :] = hybrid_img
            hybrid_vcls[i, :] = vcls_flat
            # Store class
        ...
        return [hybrid_vcls, hybrid_images], aggregated_labels
